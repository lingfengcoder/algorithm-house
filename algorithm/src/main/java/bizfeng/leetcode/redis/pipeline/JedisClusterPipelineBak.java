...
//import java.util.*;
//
///** 基于jedis-2.9版本进行实现的 jedis-4.0以上不可用
// * 在集群模式下提供批量操作的功能。 <br/>
...
// * @since Ver 1.1
// */
//public class JedisClusterPipelineBak extends Pipeline implements Closeable {
//    private static final Logger LOGGER = LoggerFactory.getLogger(JedisClusterPipelineBak.class);
//
...
//     * @return
//     */
//    public static JedisClusterPipelineBak pipelined(JedisCluster jedisCluster) {
//        JedisClusterPipelineBak pipeline = new JedisClusterPipelineBak();
//        pipeline.setJedisCluster(jedisCluster);
//        return pipeline;
//    }
//
//    public JedisClusterPipelineBak() {
//    }
//
...
//     */
//    public <T> List<T> syncAndReturnAll() {
...
//    }
//
//    private <T> void innerSync(List<T> formatted) {
//        try {
//            for (Client client : clients) {
//                // 在sync()调用时其实是不需要解析结果数据的，但是如果不调用get方法，
//                // 发生了JedisMovedDataException这样的错误应用是不知道的，因此需要调用get()来触发错误。
//                // 其实如果Response的data属性可以直接获取，可以省掉解析数据的时间，
//                // client.getOne() 负责从数据流中读取数据，generateResponse 负责封装数据
//                Object data = generateResponse(client.getOne()).get();
//                if (null != formatted) {
//                    if (data != null) {
//                        formatted.add((T) data);
//                    } else {
//                        formatted.add(null);
//                    }
//                }
...
//        byte[] bKey = SafeEncoder.encode(key);
//        return getJedis(JedisClusterCRC16.getSlot(key));
//    }
//
